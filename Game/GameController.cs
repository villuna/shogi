...
{
    [Export]
    public required PackedScene mainMenuScene;

    // Lost of child nodes that we need references to during the runtime of the program.
    // Often this is done with onready vars and GetNode() but I personally believe it's not a good
    // practice to refer to notes directly by name because it causes problems when they're renamed.
    // An alternative is to have them as exports and set them in the editor, like so.
    [Export]
    ...
    [Export]
    public required Control pauseMenu;
    [Export]
